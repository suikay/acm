...
void network::build(int m,int sr) { 
	v = 2+m; eg.clear(); 
	int a, b, i, j, cnt;  typef l; 
	char ch[4];
	for (i = 1; i <= m; i++) { 
		scanf("%s",ch);
		if(ch[1] == '\0')
			eg.push_back(edge(0,i,2*m,0));
		scanf("%d",&cnt);
		for(j = 0; j < cnt; j++) {
			scanf("%d",&v);
			v++;
			eg.push_back(edge(i,v,2*m,0));
			eg.push_back(edge(v,i,1,0));
		}
	}
	eg.push_back(edge(sr,m+1,2*m,0));
} 
