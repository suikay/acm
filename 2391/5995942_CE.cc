...
using namespace std;
const int N = 410;
#define typef __int64                   // type of flow 
const typef inf = (long long (1000000000))*1000000;     // max of flow 
...
struct network { 
	...
	void push(int); 
	void relabel(int); 
	void build(int,__int64 tmp[][N/2],int *,int *, __int64); 
...
void network::build(int f, __int64 net[N/2][N/2],int now[N],int cap[N],__int64 up_bound) { 
	v = f*2+2; eg.clear(); 
	for(int i = 1; i <= f; i++)
		for(int j = i+1; j <= f; j++)
...
{
	int u,v,f,p,sum;
	long long left,right,mid,w,ori;
	scanf("%d%d",&f,&p);
	sum = 0;
	for(int i = 1; i <= f; i++) {
	...
	left = 0; right = ori = ((__int64)f) * 1000000000 + 1;
	while(left < right)
	{
		mid = (left + right) >> 1;
