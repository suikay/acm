...
int MergeOppisiteSet(int a ,int b)
{
	...
	if( t1 == t2 || (eatParent[t1] && eatParent[t2] && eatParent[t1] == eatParent[t2]) || \
		(eatedParent[t1] && eatedParent[t2] && eatedParent[t1] == eatedParent[t2]))
		return 0;
	eatParent[t2] = MergeSet(eatParent[t2],t1);
	eatedParent[t1] = MergeSet(t2,eatedParent[t1]);
	eatParent[t1] = MergeSet(eatParent[t1],t2);
	eatedParent[t2] = MergeSet(eatParent[t1],eatedParent[t2]);
	return 1;
}
int main()
{
